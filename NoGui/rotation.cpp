...
void Rotation::rotateTowards(float goalRot) {
	if(goalRot > 360.0) {
		DBPL("[ROT] invalid goalRot: " << goalRot);
	} else if (goalRot == angle) {
		DBPL("[ROT] already at goalRot: " << goalRot);
	} else {
		float rot = distanceTo(goalRot);

		if(rot > 0) {
			DBPL("[ROT] rotating positive direction (" << angle << "->" << goalRot << ")");
		}
		else {
			DBPL("[ROT] rotating positive direction (" << angle << "->" << goalRot << ")");    			
		}
...
void Rotation::set(float a) {
	if(a < 0.0 || a > 360.0) {
		DBPL("[ROT] INVALID input to setAngle(): " << a);
	} else {
    	angle = a;
	}
}
